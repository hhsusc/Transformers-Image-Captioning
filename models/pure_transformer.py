...
class PureT(BasicModel):
    ...
    def forward(self, **kwargs):
        ...
        if type(att_feats) is BaseModelOutputWithPooling:
...
# DeiT (Data-Efficient Image Transformers) Backbone
# class PureT_DeiT(PureT):
class PureT_DeiT(BasicModel):
    def __init__(self):
        # super(PureT_DeiT, self).__init__()
        super(PureT_DeiT, self).__init__()
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1
        
        # del self.backbone
        
        # image_processor = AutoImageProcessor.from_pretrained("facebook/deit-base-distilled-patch16-384")
